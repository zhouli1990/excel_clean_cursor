...
**需求背景**：
当前实现中，数据处理逻辑导致合并后的原始数据大幅减少，无法保留完整信息。例如：当合并1101行数据（553行本地数据 + 548行飞书数据）时，最终生成的Excel中，原始数据sheet页大幅减少，飞书数据占大部分，本地数据仅保留少量条目。这是因为在`merge_duplicate_phones`函数中对重复手机号进行了过度合并。

**目标**：
优化多Sheet页Excel生成逻辑，保留原始数据的完整性，同时对"新增"和"更新"sheet页应用智能去重和合并规则。

**设计方案**：

*   **影响模块**：`postprocessor.py`中的`create_multi_sheet_excel`函数和相关调用。
*   **核心函数**：修改`create_multi_sheet_excel`函数和`app.py`中调用该函数的部分。

**详细步骤**：

1. **修改接口和参数**：
   * 为`create_multi_sheet_excel`函数添加新参数`df_original`，用于接收未经去重的原始合并数据。
   * 修改函数文档注释，明确说明三个Sheet页的不同用途和处理逻辑。

2. **修改原始数据Sheet处理**：
   * "原始数据"Sheet使用`df_original`参数中的完整数据，不进行去重或过滤。
   * 如果调用时未提供`df_original`，则退化为使用`df_processed`作为原始数据。

3. **优化新增和更新Sheet的数据处理**：
   * **新增Sheet**：
     * 仅包含`record_id`为空且电话号码格式正确的记录。
     * 对于非重复手机号的数据：直接放入新增Sheet。
     * 对于重复手机号但公司名称相同或LLM判断相关的数据：保留一条放入新增Sheet。
     * 对于重复手机号、公司名称不同且LLM判断不相关的数据：
       * 如果所有记录均无`record_id`：保留一条，合并所有不同的企业名称和来源（用分号连接），放入新增Sheet。

   * **更新Sheet**：
     * 不再包含所有`record_id`非空的记录。
     * 仅包含因合并操作（如合并企业名称、来源）而需要更新的记录。
     * 对于重复手机号、公司名称不同且LLM判断不相关的数据：
       * 如果至少有一条记录的`record_id`非空：保留一条有`record_id`的记录，合并所有不同的企业名称和来源（用分号连接），仅当数据确实需要更新时才放入更新Sheet。

4.  **在`app.py`中的修改**：
   * 在调用后处理函数之前，保存一份合并后但未去重的原始数据副本：`combined_df_original = combined_df.copy()`
   * 调用`create_multi_sheet_excel`时传入这个原始数据：`postprocessor.create_multi_sheet_excel(final_df, output_file, config, df_original=combined_df_original)`

**预期效果**：
* "原始数据"Sheet保留合并后的完整数据（如1101行），不受去重影响。
* "新增"Sheet只包含经过智能筛选的、真正需要新增的记录，手机号重复但公司不同的情况会合并企业名称和来源。
* "更新"Sheet只包含因企业名称/来源合并而确实需要更新的记录，避免不必要的飞书API调用。
* 整体维持数据完整性，同时实现更高效的飞书同步。 
