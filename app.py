    ...
    try:
        ...
        if not merged_df.empty and post_choices:
            try:
                ...
                # 保存最终结果
                final_df.to_excel(final_output_path, index=False, engine="openpyxl")
                print(
                    f"任务 {task_id}: 后处理完成，最终结果保存在: {final_output_path}"
                )
                update_task_progress(
                ...
                try:
                    ...
                    print(f"   合并后的数据已保存到: {final_output_path}")
            ...
            try:
                final_df.to_excel(final_output_path, index=False, engine="openpyxl")
                final_output_filename = final_output_filename  # 保持最终文件名
                print(f"   合并结果已直接保存为最终文件: {final_output_path}")
...
def check_differences(task_id):
    ...
    # 检查必要的文件是否存在
    final_filename = task_info.get("result_file")  # 这是 Stage 4/5 生成的最终文件名
    edited_file_uploaded = task_info.get("edited_file_uploaded", False)

    if not final_filename:
        return jsonify({"success": False, "error": "找不到原始处理结果文件信息。"}), 404

    if not edited_file_uploaded:
        return jsonify({"success": False, "error": "用户尚未上传调整后的文件。"}), 400

    output_dir = os.path.join(app.config["OUTPUT_FOLDER"], task_id)
    final_filepath = os.path.join(output_dir, final_filename)
    edited_filepath = task_info.get("edited_file_path")  # 从任务信息获取路径

    if not edited_filepath or not os.path.exists(edited_filepath):
        # 如果路径丢失或文件被删除，尝试重新构建路径
        ...
        if os.path.exists(edited_filepath_fallback):
            ...
            tasks[task_id]["edited_file_path"] = edited_filepath  # 更新缓存的路径
        else:
            print(
                f"错误: 找不到任务 {task_id} 的修改后文件，尝试路径: {edited_filepath_fallback}"
            )
            return jsonify({"success": False, "error": "找不到调整后的文件。"}), 404

    if not os.path.exists(final_filepath):
        print(f"错误: 找不到任务 {task_id} 的原始结果文件: {final_filepath}")
        return jsonify({"success": False, "error": "找不到原始结果文件。"}), 404

    print(f"任务 {task_id}: 开始比较文件: '{final_filepath}' 和 '{edited_filepath}'")

    try:
        # 读取 Excel 文件
        df_final = pd.read_excel(final_filepath, engine="openpyxl")
        df_edited = pd.read_excel(edited_filepath, engine="openpyxl")

        print(f"  文件读取成功: final({len(df_final)}行), edited({len(df_edited)}行)")

        # --- 获取比较所需参数 --- #
        # Define key column names
        local_id_col = "local_row_id"  # Primary key for comparison
        feishu_id_col = "record_id"  # Contextual ID

        # Check if local_id_col exists in both dataframes (basic validation)
        if (
            local_id_col not in df_final.columns
            or local_id_col not in df_edited.columns
        ):
            print(f"错误: 关键比较列 '{local_id_col}' 未在两个文件中都找到。")
            return (
                jsonify(
                    {"success": False, "error": f"比较所需列 '{local_id_col}' 缺失。"}
                ),
                400,
            )

        # No need to determine local_keys or complex columns_to_compare here anymore.
        # The compare_dataframes_hybrid function will handle determining the columns
        # to compare based on common columns, excluding the key columns.

        print(
            f"  准备调用 v2 比较函数，主键: '{local_id_col}', 外部ID: '{feishu_id_col}'"
        )

        # 调用新的差异比较函数 V2
        diff_results = diff_utils.compare_dataframes_hybrid(
            df_final=df_final,
            df_edited=df_edited,
            local_id_column=local_id_col,  # Pass the defined local ID column name
            id_column=feishu_id_col,  # Pass the defined Feishu ID column name
            columns_to_compare=None,  # Let the function determine columns to compare (common cols minus keys)
        )

        # 将比较结果保存到任务状态，方便后续分页获取
        tasks[task_id]["diff_results"] = diff_results

        print(f"任务 {task_id}: 差异比较完成。Summary: {diff_results.get('summary')}")

        # 初始返回第一页的数据 (或其他必要信息)
        # TODO: 实现分页逻辑，这里先返回完整结果 (可能很大!)
        return jsonify({"success": True, "results": diff_results})

    except FileNotFoundError as fnf_err:
        print(f"任务 {task_id}: 读取比较文件时出错: {fnf_err}")
        return (
            jsonify({"success": False, "error": f"读取文件失败: {fnf_err.filename}"}),
            500,
        )
    except Exception as e:
        print(f"任务 {task_id}: 执行差异比较时发生错误: {e}")
        traceback.print_exc()
        return (
            jsonify({"success": False, "error": "执行差异比较时发生服务器错误。"}),
            500,
        )
...
def sync_to_feishu(task_id):
    ...
    task_info = tasks[task_id]
    print(
        f"任务 {task_id}: 开始执行同步到飞书操作 (新增来自文件, 修改/删除来自差异)..."
    )

    # --- 检查必需的信息和配置 --- #
    edited_filepath = task_info.get("edited_file_path")
    diff_results = task_info.get("diff_results")  # 现在需要 diff_results

    if not edited_filepath or not os.path.exists(edited_filepath):
        # Edited 文件必须存在，因为新增依赖它
        output_dir = os.path.join(app.config["OUTPUT_FOLDER"], task_id)
        edited_filename_fallback = f"edited_{task_id}.xlsx"
        edited_filepath_fallback = os.path.join(output_dir, edited_filename_fallback)
        if os.path.exists(edited_filepath_fallback):
            ...
            print(
                f"错误: 找不到任务 {task_id} 的调整后文件: {edited_filepath_fallback}"
            )
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "找不到调整后的文件，无法执行新增操作。",
                    }
                ),
                404,
            )
    ...
    if not diff_results:
        # diff_results 也必须存在，因为修改和删除依赖它
        return (
            jsonify(
                {
                    "success": False,
                    "error": "未找到差异比较结果，无法执行修改/删除操作。请先执行检查差异。",
                }
            ),
            400,
        )
    ...
    # target_columns 的验证放到后面，因为 diff_results 可能提供回退方案
    else:
        msg = None
    if msg:
        print(f"❌ 配置错误: {msg}")
    ...
    local_id_col = "local_row_id"  # 虽然新增不基于它，但 diff_results 里有
    ...
    # --- 确定目标列 (如果配置为空，尝试从 diff_results 获取) --- #
    if not target_columns:
        print("   ⚠️ 警告: LLM 配置中 TARGET_COLUMNS 为空，尝试从 diff_results 推断...")
        all_cols = diff_results.get("columns", [])
        target_columns = [c for c in all_cols if c not in [local_id_col, feishu_id_col]]
        if not target_columns:
            msg = "无法确定要同步到飞书的目标列名。"
            print(f"❌ {msg}")
            flash(msg, "error")
            return jsonify({"success": False, "error": msg}), 500
        print(f"   -> 使用 Diff 结果中的列作为目标列: {target_columns}")

    try:
        # --- 1. 处理新增 (从 edited 文件读取) --- #
        print(f"  步骤 1: 从 '{edited_filepath}' 读取数据以识别新增...")
        try:
            df_edited = pd.read_excel(edited_filepath, engine="openpyxl")
            print(f"    -> 调整后文件读取完成 ({len(df_edited)} 行)。")

            # 确保 record_id 列存在
            if feishu_id_col not in df_edited.columns:
                print(f"    -> 文件中无 '{feishu_id_col}' 列，所有行都将被视为新增。")
                df_edited[feishu_id_col] = None
            else:
                # 清理 record_id
                df_edited[feishu_id_col] = (
                    df_edited[feishu_id_col].fillna("").astype(str).str.strip()
                )

            # 筛选需要新增的行 (record_id 为空)
            rows_to_add_df = df_edited[df_edited[feishu_id_col] == ""]
            print(f"    -> 文件中找到 {len(rows_to_add_df)} 行 record_id 为空的数据。")

            for index, row in rows_to_add_df.iterrows():
                add_payload = {"fields": {}}
                for col in target_columns:
                    if col in row.index:
                        value = row[col]
                        # *** 修改开始: 忽略 None 值，并将所有值转为字符串 ***
                        if pd.isna(value) or value is None:
                            continue  # 跳过 None 或 NaN 值

                        # 处理列表/字典类型 -> 转为 JSON 字符串
                        if isinstance(value, (list, dict)):
                            value_str = json.dumps(value, ensure_ascii=False)
                        # 处理其他类型 (数字、布尔、已是字符串等) -> 直接转字符串
                        else:
                            value_str = str(value)

                        add_payload["fields"][col] = value_str  # 存储字符串格式的值
                        # *** 修改结束 ***

                # 只有当 fields 字典非空时才添加到列表 (如果一行所有目标列都为 None, 则不添加)
        ...
        # --- 2. 处理修改和删除 (从 diff_results 读取) --- #
        print(f"  步骤 2: 从差异结果 (diff_results) 中识别修改和删除...")
        diff_items = diff_results.get("diffs", [])
        print(f"    -> 差异结果包含 {len(diff_items)} 条记录。")

        for diff_item in diff_items:
            status = diff_item.get("status")
            record_id = diff_item.get(feishu_id_col)

            # 处理删除
            if status == "Deleted":
                if record_id and pd.notna(record_id) and str(record_id).strip():
                    records_to_delete.append(str(record_id).strip())
                else:
                    local_row_id = diff_item.get(local_id_col, "未知")
                    print(
                        f"    -> 忽略 'Deleted' 状态，因缺少有效 record_id (local_id: {local_row_id})."
                    )

            # 处理修改
            elif status == "Modified":
                if record_id and pd.notna(record_id) and str(record_id).strip():
                    changes = diff_item.get("changes", {})
                    update_fields = {}
                    for col, change_detail in changes.items():
                        if col in target_columns:  # 只包含目标列的变更
                            new_value = change_detail.get("new")
                            # Handle types
                    ...
                    if update_fields:  # 只有当目标列确实有变化时才添加
                    ...
                    local_row_id = diff_item.get(local_id_col, "未知")
                    print(
                        f"    -> 忽略 'Modified' 状态，因缺少有效 record_id (local_id: {local_row_id})."
                    )

            # 忽略 'Added' 状态的 diff item，因为新增完全由文件决定
            # elif status == "Added":
            #    pass

        print(
            f"    -> 从差异结果中识别: 待删除={len(records_to_delete)}, 待更新={len(records_to_update)}."
        )

        # --- 3. 执行飞书API操作 --- #
        # 这部分逻辑 (调用删除、更新、新增API，包括行数检查) 大体不变
        print(f"\n  步骤 3: 开始执行飞书 API 操作...")

        # *** 新增：在开始时获取 access_token ***
        try:
        ...
        update_delete_table_id = primary_table_ids[0]
        print(f"    目标表格 (更新/删除): {update_delete_table_id}")
        final_add_target_table_id = None
        records_to_add_validated = records_to_add.copy()

        # --- 批量删除 ---
        if records_to_delete:
            print(f"      尝试删除 {len(records_to_delete)} 条记录...")
            try:
                # 注意： batch_delete_records 内部自己获取 token，所以这里不需要传 access_token
                delete_result = feishu_utils.batch_delete_records(
                    app_token,
                    update_delete_table_id,
                    records_to_delete,
                    app_id,
                    app_secret,
                )
                results["deleted"] = delete_result.get("success_count", 0)
                results["delete_errors"] = delete_result.get(
                    "error_count", len(records_to_delete) - results["deleted"]
                )
                if delete_result.get("errors"):
                    results["errors"].extend(
                        [f"删除失败: {e}" for e in delete_result["errors"]]
                    )
                print(
                    f"        删除结果: 成功 {results['deleted']}, 失败 {results['delete_errors']}"
                )
            except Exception as del_err:
                print(f"     ❌ 调用批量删除时发生错误: {del_err}")
                results["delete_errors"] += len(records_to_delete)
                results["errors"].append(f"批量删除API调用失败: {del_err}")

        # --- 批量更新 ---
        if records_to_update:
            print(f"      尝试更新 {len(records_to_update)} 条记录...")
            try:
                # 注意： batch_update_records 内部自己获取 token，所以这里不需要传 access_token
        ...
        if records_to_add_validated:
            ...
            if add_target_table_ids:
                ...
                for target_id in add_target_table_ids:
                    ...
                    try:
                        # *** 修改：调用 get_table_record_count 并传入正确的参数 ***
                ...
                target_id = update_delete_table_id
            ...
            if records_to_add_validated and target_found and final_add_target_table_id:
                ...
                try:
                    # 注意： batch_add_records 内部自己获取 token，所以这里不需要传 access_token
    ...
    # --- 组装最终响应 --- #
    total_success = results["deleted"] + results["updated"] + results["added"]
    total_errors = (
        results["delete_errors"] + results["update_errors"] + results["add_errors"]
    )
    total_errors += len(
        [
            e
    ...
    success_msg = f"同步完成: 新增 {results['added']} 条, 更新 {results['updated']} 条, 删除 {results['deleted']} 条。"
    if total_errors > 0:
        error_details = []
        if results["delete_errors"] > 0:
            error_details.append(f"删除失败 {results['delete_errors']} 条")
